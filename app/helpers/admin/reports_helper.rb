module Admin::ReportsHelper
    def total_reviews
		...
		@compliments = Review.where('created_at >= ? and created_at <= ? and user_id is not null and review_type = ?', 1.year.ago, Date.today,'compliment')  rescue nil
		@complaints = Review.where('created_at >= ? and created_at <= ? and user_id is not null and review_type = ?', 1.year.ago, Date.today,'complaint')  rescue nil
		@total_converted = Review.where('created_at >= ? and created_at <= ? and user_id is not null and review_type = ? and change_date is not null', 1.year.ago, Date.today,'compliment')  rescue nil
	end
	...
	def industry_level #industries
        ...
        #nature of review for compliments
		@billing = Review.where('created_at >= ? and created_at <= ? and industry_id = ? and user_id is not null and nature_of_review = ? and review_type = ?  ',1.year.ago, Date.today,  @industry.id ,'Billing/accounts', 'compliment').count rescue nil
   ...
   def most_company_xls
      ...
      if params[:subaction] == "most_company"
        ...
        @most_compliment = Review.select(:company_id).where('created_at >= ? and created_at <= ? and review_type = ? and user_id is not null',@start_from, @start_to, 'compliment').map(&:company_id) rescue nil
   	    freq = @most_compliment.inject(Hash.new(0)) { |h,v| h[v] += 1; h }
   	    @value_compliments = @most_compliment.max_by { |v| freq[v] }
   	    @max_compliment = freq.values.max
   	    @compliments = Review.where('created_at >= ? and created_at <= ? and user_id is not null and company_id = ? and review_type = ?',@start_from, @start_to,@value_compliments, 'compliment') rescue nil
        # Most complaints for supplier
   	    @most_complaint = Review.select(:company_id).where('created_at >= ? and created_at <= ? and review_type = ? and user_id is not null',@start_from, @start_to, 'complaint').map(&:company_id) rescue nil
   	    freq = @most_complaint.inject(Hash.new(0)) { |h,v| h[v] += 1; h }
   	    @value_complaints = @most_complaint.max_by { |v| freq[v] }
   	    @max_complaint = freq.values.max
   	    @complaints = Review.where('created_at >= ? and created_at <= ? and user_id is not null and company_id = ? and review_type = ?',@start_from, @start_to,@value_complaints, 'complaint') rescue nil
